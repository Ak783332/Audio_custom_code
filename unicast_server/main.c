...
static void button_msg_sub_thread(void)
{
	...
	while (1) {
		...
		case BUTTON_PLAY_PAUSE: {
			...
			static int64_t last_tap_time = 0; // Timestamp of the last tap

			int64_t current_time =
				k_uptime_get(); // Get the current uptime in milliseconds

			// Reset tap count if the last tap was too long ago (e.g., > 500ms)
			if ((current_time - last_tap_time) > 300) {
				LOG_DBG("Tap sequence timeout, processing action for %d taps",
					tap_count);

				// Perform the action based on the final tap count
				switch (tap_count) {
				case 1:
				...
				case 5:
					...
					break;
				default:
					LOG_WRN("Invalid tap count: %d", tap_count);
				}
			...
			tap_count++;		      // Increment the tap count
#else
...
// 		case BUTTON_PLAY_PAUSE:
// 			if (IS_ENABLED(CONFIG_STREAM_BIDIRECTIONAL)) {
// 				LOG_WRN("Play/pause not supported in "
// 					"walkie-talkie/bidirectionalmode");
// 				break;
// 			}
