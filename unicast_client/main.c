...
#define TAP_TIMEOUT_MS 300  // Timeout in milliseconds

static int tap_count = 0;         // Counter for button taps

// Timer handler function
static void tap_timeout_handler(struct k_timer *dummy) {
    if (tap_count > 0) {
        LOG_DBG("Tap sequence timeout, processing %d taps", tap_count);

        // Perform action based on the tap count
        switch (tap_count) {
            case 1:
                LOG_INF("Switching to User 1");
                // Add code to switch to User 1
                break;
            case 2:
                LOG_INF("Switching to User 2");
                // Add code to switch to User 2
                break;
            case 3:
                LOG_INF("Switching to User 3");
                // Add code to switch to User 3
                break;
            case 4:
                LOG_INF("Switching to User 4");
                // Add code to switch to User 4
                break;
            case 5:
                LOG_INF("Switching to User 5");
                // Add code to switch to User 5
                break;
            default:
                LOG_WRN("Invalid tap count: %d, resetting", tap_count);
                break;
        }
...
// Define the timer using Zephyr's k_timer API
K_TIMER_DEFINE(tap_timer, tap_timeout_handler, NULL);
...
static void button_msg_sub_thread(void)
{
	...
	while (1) {
		...
		case BUTTON_PLAY_PAUSE: {
			...
			tap_count++;
			LOG_DBG("Tap detected, count: %d", tap_count);

			// Restart the timer on each tap
			k_timer_stop(&tap_timer); // Stop the timer if it's already running
			k_timer_start(&tap_timer, K_MSEC(TAP_TIMEOUT_MS),
				      K_NO_WAIT); // Start the timer

#else
			// Normal play/pause functionality when bidirectional mode is disabled
